...
L = {
    "lang_uz_cyr": {"ok": "✅ Тил танланди!", "wait": "⏳ Жавоб тайёрланмоқда...", "warn": "⚠️ Жавоблар умумий ва таълимий мақсадда.", "err": "❌ Хатолик юз берди.", "api": "❌ API калит нотўғри."},
    "lang_uz_lat": {"ok": "✅ Til tanlandi!", "wait": "⏳ Javob tayyorlanmoqda...", "warn": "⚠️ Javoblar umumiy va ta'limiy maqsadda.", "err": "❌ Xatolik yuz berdi.", "api": "❌ API kalit noto'g'ri."},
    "lang_ru": {"ok": "✅ Язык выбран!", "wait": "⏳ Ответ готовится...", "warn": "⚠️ Ответы носят ознакомительный характер.", "err": "❌ Произошла ошибка.", "api": "❌ Ошибка API ключа."},
}
...
async def language_chosen(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    prompt = {
        ...
        "lang_ru": "\n\nПишите ваш вопрос по госзакупкам, законодательству или финансам:",
    }
    await query.edit_message_text(f"{msg}{prompt.get(lang, '')}")
...
4. Если не уверены — напишите: Обратитесь к официальному источнику"""
    ...
    try:
        ...
        await status_msg.edit_text(L[lang]["api"])
    except anthropic.RateLimitError:
        await status_msg.edit_text(L[lang]["err"])
    except Exception as e:
        print(f"XATO: {e}")
        await status_msg.edit_text(f"{L[lang]['err']} {str(e)[:50]}")
